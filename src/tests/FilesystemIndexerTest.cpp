...
TEST_CASE_METHOD(FilesystemIndexerFixture, "IndexesCorrectNumberOfFiles",
                 "[FilesystemIndexer]") {
  size_t filesIndexed = indexer.indexDirectory(testDir.string(), 2);
  REQUIRE(filesIndexed == 4);
}

TEST_CASE_METHOD(FilesystemIndexerFixture, "RespectsDepthParameter",
                 "[FilesystemIndexer]") {
  size_t shallowIndex = indexer.indexDirectory(testDir.string(), 1);
  REQUIRE(shallowIndex == 2);
}

TEST_CASE_METHOD(FilesystemIndexerFixture, "FindsFilesWithGivenSuffixes",
                 "[FilesystemIndexer]") {
...
TEST_CASE_METHOD(FilesystemIndexerFixture,
                 "HandlesNonExistentDirectoryGracefully",
                 "[FilesystemIndexer]") {
  size_t filesIndexed =
      indexer.indexDirectory(testDir.string() + "/nonexistent", 2);
  REQUIRE(filesIndexed == 0);
}

TEST_CASE_METHOD(FilesystemIndexerFixture,
                 "Depth -1 Traverses the Full Hierarchy",
                 "[FilesystemIndexer]") {
...
TEST_CASE_METHOD(FilesystemIndexerFixture, "Depth 0 Does Not Traverse",
                 "[FilesystemIndexer]") {
