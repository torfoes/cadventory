...
ProcessGFiles::executeMultiThreadedProcessing(
    const std::vector<std::string>& allGeometry, int num_workers) {
  ...
  for (int i = 0; i < num_workers; ++i) {
    threads.emplace_back(&ProcessGFiles::gFileWorker, this,
                         std::ref(file_queue));
  }
...
std::pair<string, string> ProcessGFiles::runCommand(
...
std::map<std::string, std::string> ProcessGFiles::processGFile(
  ...
  try {
    ...
    vector<string> objects_to_try = {"all", "all.g", model_short_name,
                                     model_short_name + ".g",
                                     model_short_name + ".c"};
    ...
    string root_folder =
        fs::current_path()
            .string();  // Get the current root folder of the project
    ...
    for (const auto& obj : objects_to_try) {
      ...
      try {
        ...
        this_thread::sleep_for(
            chrono::milliseconds(100));  // Ensure file is written properly
        if (fs::exists(png_file) && fs::file_size(png_file) > 0) {
          raytrace_successful = true;
          cout << "Successfully created preview for: " << model_short_name
               << endl;
          break;
        }
      } catch (const exception& e) {
        cerr << "Raytrace failed for '" << obj << "', trying next object..."
             << endl;
      }
    }

    if (!raytrace_successful) {
      cerr << "Raytrace failed for all possible objects in " << model_short_name
           << "." << endl;
